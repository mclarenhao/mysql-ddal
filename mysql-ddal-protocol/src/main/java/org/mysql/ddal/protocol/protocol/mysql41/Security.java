...
public class Security {
	...
	public static byte[] scramble411(byte[] password, byte[] seed, String passwordEncoding) {
		byte[] passwordBytes = (passwordEncoding == null || passwordEncoding.length() == 0) ? password : password;
	...
	public static byte[] scrambleCachingSha2(byte[] password, byte[] seed) throws DigestException {
		/*
		 * Server does it in 4 steps (see sql/auth/sha2_password_common.cc
		 * Generate_scramble::scramble method):
		 * 
		 * SHA2(src) => digest_stage1 SHA2(digest_stage1) => digest_stage2
		 * SHA2(digest_stage2, m_rnd) => scramble_stage1 XOR(digest_stage1,
		 * scramble_stage1) => scramble
		 */
		MessageDigest md;
		try {
